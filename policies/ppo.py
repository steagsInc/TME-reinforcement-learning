...
def main():
    ...
    traced = torch.jit.script(agent.anet)
    torch.jit.save(traced, '../data/policies/Pendulum-v0#PPO#ppo#200#1000.zip')

    plt.plot([r.ep for r in training_records], [r.reward for r in training_records])
    plt.title('PPO')
    plt.xlabel('Episode')
    plt.ylabel('Moving averaged episode reward')
    plt.savefig("../data/plots/ppo.png")
    plt.show()
